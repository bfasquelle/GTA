...
\begin{frame}{NPG in single commodity}
  ...
  \begin{definition}[Monopoly]
    \emph{Context: Network}. An edge is monopolized over a user if its removal causes the non-reachability of the destination.
  \end{definition}
\end{frame}
...
\begin{frame}{Multiple monopolies}
  ...
  \end{theorem}
  \begin{theorem}
    There exists a family of single-commodity instances with \(POS_{Val}\), \(POS_{Pro} = \Omega(\mathcal{L}^{k-1})\), where \(k\) is the number of monopolies. Moreover, in all single-commodity graphs with \(k > 1\) monopolies, \(POS_{Val}\), \(POS_{Pro} = O(\mathcal{L}^{k-1})\).
  \end{theorem}
\end{frame}
